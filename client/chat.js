...
var chat = {
	...
	send: function() {
		...
		document.getElementById('chat_list').innerHTML += date.getHours()+":"+(date.getMinutes()<10?'0':'') + date.getMinutes()+" - "+player.name+"."+player.id+" (You) say: "+chat_message+"<br />";
	...
	display: function(data) {
		if (typeof data.name != 'undefined') {
			document.getElementById('chat_list').innerHTML += data.send_date+' - '+ data.name + ' says: ' + data.message + '<br />';
			document.getElementById('latest-chat').innerHTML = data.send_date+' - '+ data.name + ' says: ' + data.message + '<br />';
		} else {
